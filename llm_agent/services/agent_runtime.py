...
from ..skills.registry import build_skill_context_for_tools, execute_skill, get_tool_definitions
...
class AgentRuntime:
    ...
    def run(self, messages: list[dict[str, Any]]) -> AgentRunResult:
        ...
        tool_calls = self.client.extract_tool_calls(payload, interface=interface)
        selected_skill_context = build_skill_context_for_tools([tool_call.name for tool_call in tool_calls])
    ...
    def _run_responses(
        ...
        *,
        ...
        for _ in range(self.max_tool_rounds + 1):
            ...
            response_input = []
            if selected_skill_context and not skill_context_injected:
            ...
            for tool_call in tool_calls:
                ...
                result = execute_skill(tool_call.name, arguments, self.skill_context)
                response_input.append(
                    {
