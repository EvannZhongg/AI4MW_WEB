...
from paper_saerch.app.domain.schemas import (
    CriterionJudgment,
    PaperResult,
    QueryBundleItem,
    RetrievalTrace,
...
from paper_saerch.app.prompts import INTENT_PLANNER_SYSTEM_PROMPT, INTENT_PLANNER_USER_PROMPT, render_prompt
from paper_saerch.app.services.progress import ProgressReporter, emit_progress, serialize_paper_result
from paper_saerch.app.services.provider_registry import get_clients_for_mode
from paper_saerch.config import get_settings


TOKEN_PATTERN = re.compile(r"[A-Za-z0-9]+|[\u3400-\u4DBF\u4E00-\u9FFF]+")
CONJUNCTION_PATTERN = re.compile(
    r"\b(?:and|plus|with|along with|together with|combined with|hybrid|joint)\b|和|与|及|以及|结合|联合|融合|混合",
    re.IGNORECASE,
)
...
COMBINATION_HINT_PATTERN = re.compile(
    r"\b(?:combine|combined|combining|hybrid|mixed|integrated|joint)\b|结合|联合|融合|混合",
    re.IGNORECASE,
)
...
def _extract_description_search_phrases(description: str) -> list[str]:
    ...
    if re.search(r"\b(?:hybrid|combine|combined|joint|integrat|coordinated)\b", lowered):
...
def _build_provider_friendly_query_hints(criterion: SearchCriterion) -> list[str]:
    ...
    if _is_combination_criterion(criterion):
        ...
        if any(keyword in haystack for keyword in ("hybrid", "combine", "combined", "joint", "integrat", "mixed")):
...
async def recall_results_by_source(
    ...
    tasks = [asyncio.create_task(timed_batch_search(client)) for client in clients]
    ...
    for completed in asyncio.as_completed(tasks):
        client_name, client_payload, client_elapsed_ms = await completed
        ...
        if source_results:
            ...
            deduped_source_results = dedup_results(source_results)
            results_by_source[client_name] = deduped_source_results
            await emit_progress(
                reporter,
                "source_results",
                source=client_name,
                count=len(deduped_source_results),
                items=[serialize_paper_result(item) for item in deduped_source_results[:12]],
            )
        else:
            await emit_progress(
                reporter,
                "status",
                stage="recall",
                message=f"{client_name} 检索完成，未找到候选论文。",
            )
