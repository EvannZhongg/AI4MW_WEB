...
from paper_saerch.app.domain.schemas import PaperResult, SearchRequest, SearchResponse
from paper_saerch.app.llm import EmbeddingClient
...
async def run_quick_channel(
    ...
    await emit_progress(reporter, "status", stage="query_bundle", message="正在生成检索表达式...")
    step_started_at = time.perf_counter()
    ...
    await emit_progress(
        ...
        "status",
        stage="dedup",
        message=f"已合并候选论文，当前共有 {len(deduped)} 篇待排序。",
    )

    await emit_progress(reporter, "status", stage="ranking", message="正在计算相关性与排序分数...")
    step_started_at = time.perf_counter()
    ...
    await emit_progress(
        ...
        mode="quick",
        final=False,
        items=[serialize_paper_result(item) for item in ranked[:20]],
    )
