...
from paper_saerch.app.domain.schemas import CriterionJudgment, PaperResult, SearchCriterion, SearchIntent, SearchRequest, SearchResponse
from paper_saerch.app.llm import LLMClient
from paper_saerch.app.prompts import DEEP_JUDGE_SYSTEM_PROMPT, DEEP_JUDGE_USER_PROMPT, render_prompt
from paper_saerch.app.services.progress import (
    ProgressReporter,
    emit_progress,
    serialize_intent,
    serialize_judgment,
    serialize_paper_result,
    serialize_query_bundle,
)
from paper_saerch.app.services.search_common import (
    add_timing_ms,
...
async def _apply_llm_batch(
    ...
    async def _judge_candidate(result: PaperResult):
        try:
            return result, await _llm_judge(query, intent, result, llm_client, channel_settings)
        except Exception as exc:  # noqa: BLE001
            return result, exc

    tasks = [asyncio.create_task(_judge_candidate(result)) for result in candidates]
    for completed in asyncio.as_completed(tasks):
        result, payload = await completed
        ...
        if isinstance(payload, Exception):
            ...
            await emit_progress(
                ...
                paper=serialize_paper_result(result),
                criteria=[serialize_judgment(item) for item in result.criterion_judgments],
                fallback=True,
            )
            continue
        ...
        await emit_progress(
            ...
            source=result.source,
            paper=serialize_paper_result(result),
            criteria=[serialize_judgment(item) for item in merged_judgments],
            fallback=False,
        )
...
async def _judge_source_results(
    ...
    await emit_progress(
        ...
        "status",
        stage="judge",
        message=f"{source_name} 已完成初筛，待判断 {len(candidates)} 篇候选论文。",
    )
...
async def run_deep_channel(
    ...
    await emit_progress(reporter, "status", stage="planning", message="正在解析检索意图...")
    step_started_at = time.perf_counter()
    intent = await plan_search_intent(request.query, request)
    timings_ms["plan_intent"] = elapsed_ms(step_started_at)
    await emit_progress(reporter, "intent", intent=serialize_intent(intent))
    channel_settings = get_channel_settings("deep")

    await emit_progress(reporter, "status", stage="query_bundle", message="正在生成深度检索表达式...")
    step_started_at = time.perf_counter()
    query_bundle = build_query_bundle("deep", request, intent)
    timings_ms["build_query_bundle"] = elapsed_ms(step_started_at)
    await emit_progress(reporter, "query_bundle", items=serialize_query_bundle(query_bundle))

    await emit_progress(reporter, "status", stage="recall", message="正在从多个学术源召回候选论文...")
    step_started_at = time.perf_counter()
    ...
    await emit_progress(reporter, "status", stage="judge", message="正在逐篇评估候选论文...")
    step_started_at = time.perf_counter()
    judge_tasks = [
        asyncio.create_task(
            _judge_source_results(
                request.query,
                intent,
                source_name,
                source_results,
    ...
    for completed in asyncio.as_completed(judge_tasks):
        source_name, source_results, source_timings_ms = await completed
        ...
        partial_ranked = dedup_results(merged_results)
        partial_ranked.sort(
            key=lambda item: (
        ...
        await emit_progress(
            ...
            mode="deep",
            final=False,
            items=[serialize_paper_result(item) for item in partial_ranked[:20]],
        )
    ...
    await emit_progress(
        ...
        mode="deep",
        final=False,
        items=[serialize_paper_result(item) for item in deduped[:20]],
    )
